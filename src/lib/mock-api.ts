...
export async function mockGetCategories(): Promise<NewsArticle["category"][]> {
  await delay(50);
  return categoriesData;
}

export async function mockGetLatestNews(count: number = 3): Promise<NewsArticle[]> {
  await delay(100);
  return newsData
    .sort(
      (a, b) =>
...
export async function mockGetNewsList(
  page: number = 1,
  categorySlug?: string,
  perPage: number = 6
...
export async function mockGetNewsBySlug(
  slug: string
...
export async function mockGetPageBySlug(slug: string): Promise<Page | null> {
  await delay(100);
